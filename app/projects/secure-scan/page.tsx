      ...
      <section className="relative pt-28 sm:pt-36">
        ...
        <div className="relative mx-auto max-w-7xl px-5 py-16 sm:px-6 sm:py-24 lg:px-8 lg:py-28">
          <div className="max-w-3xl">
            ...
            <h1 className="mt-5 font-[var(--font-space-grotesk)] text-5xl font-semibold leading-[0.95] tracking-[-0.07em] sm:text-7xl">
              See the security posture of a website before it becomes a problem.
            </h1>
            <p className="mt-7 max-w-2xl text-base leading-7 text-[var(--muted)] sm:text-lg">
              A focused security assessment covering HTTPS, certificates, domain email protection, technology exposure, and discovered weaknesses.
            </p>
          </div>
          ...
          <div className="mt-12 max-w-5xl rounded-[2rem] border border-[var(--border)] bg-[var(--surface)]/80 p-5 shadow-[0_24px_80px_rgba(0,0,0,0.08)] backdrop-blur-xl sm:p-7">
            <div className="flex flex-col gap-4 lg:flex-row">
              ...
              <input
                ...
                onKeyDown={(event) => {
                  if (event.key === "Enter") runAssessment();
                }}
                placeholder="https://company.com"
                className="min-h-14 flex-1 rounded-2xl border border-[var(--border)] bg-[var(--background)] px-5 text-[var(--text)] outline-none transition placeholder:text-[var(--muted)] focus:border-[var(--accent)] focus:ring-4 focus:ring-blue-500/10"
              />
              <button
                type="button"
                onClick={runAssessment}
                disabled={loading}
                className="min-h-14 rounded-2xl bg-[var(--text)] px-7 font-semibold text-[var(--background)] transition hover:-translate-y-0.5 disabled:cursor-wait disabled:opacity-60"
              >
          ...
          <div className="mx-auto max-w-7xl space-y-6 px-5 sm:px-6 lg:px-8">
            <div className="grid gap-6 lg:grid-cols-[0.7fr_1.3fr]">
              <div className="rounded-[2rem] border border-[var(--border)] bg-[var(--surface)] p-7 sm:p-9">
                ...
                <div className="mt-6 flex items-end gap-3">
                  <span className="font-[var(--font-space-grotesk)] text-7xl font-semibold tracking-[-0.08em]">{assessment.overview.overallScore}</span>
                  <span className="mb-2 text-sm text-[var(--muted)]">/ 100</span>
                </div>
              ...
              <div className="rounded-[2rem] border border-[var(--border)] bg-[var(--surface)] p-7 sm:p-9">
                ...
                <div className="mt-8 flex flex-wrap gap-3">
                  <span className={`rounded-full px-4 py-2 text-xs font-semibold ${statusTone(assessment.checks?.https)}`}>
                    HTTPS {assessment.checks?.https ? "enabled" : "missing"}
                  </span>
                  <span className="rounded-full bg-[var(--background)] px-4 py-2 text-xs font-semibold text-[var(--muted)]">
                    HTTP status {assessment.checks?.status || "unknown"}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="grid gap-6 lg:grid-cols-2">
              <article className="rounded-[2rem] border border-[var(--border)] bg-[var(--surface)] p-7 sm:p-9">
                <div className="flex items-center justify-between gap-4">
                  ...
                  <span className={`rounded-full px-4 py-2 text-xs font-semibold ${statusTone(assessment.ssl?.valid)}`}>
                    {assessment.ssl?.valid ? "Valid" : "Risk"}
                  </span>
                </div>
              ...
              <article className="rounded-[2rem] border border-[var(--border)] bg-[var(--surface)] p-7 sm:p-9">
                ...
                <div className="mt-8 flex flex-wrap gap-3">
                  <span className={`rounded-full px-4 py-2 text-xs font-semibold ${statusTone(assessment.domain?.emailSecurity?.SPF)}`}>SPF {assessment.domain?.emailSecurity?.SPF ? "enabled" : "missing"}</span>
                  <span className={`rounded-full px-4 py-2 text-xs font-semibold ${statusTone(assessment.domain?.emailSecurity?.DMARC)}`}>DMARC {assessment.domain?.emailSecurity?.DMARC ? "enabled" : "missing"}</span>
                </div>
            ...
            <article className="rounded-[2rem] border border-[var(--border)] bg-[var(--surface)] p-7 sm:p-9">
              ...
              <div className="mt-8 grid gap-8 md:grid-cols-3">
                {Object.entries(assessment.technology?.categories || { frontend: [], backend: [], infrastructure: [] }).map(([category, items]) => (
                  <div key={category}>
                    <p className="text-xs font-semibold uppercase tracking-[0.16em] text-[var(--muted)]">{category}</p>
                    <div className="mt-4 flex flex-wrap gap-2">
                      {safeArray(items).length ? safeArray(items).map((item) => (
                        <span key={item} className="rounded-full border border-[var(--border)] bg-[var(--background)] px-3 py-2 text-sm">{item}</span>
            ...
            <article className="rounded-[2rem] border border-[var(--border)] bg-[var(--surface)] p-7 sm:p-9">
              ...
              <div className="mt-8 space-y-3">
                {safeArray(assessment.vulnerabilities).length === 0 && safeArray(assessment.findings).length === 0 ? (
                  <div className="rounded-2xl bg-emerald-500/10 p-5 text-sm font-medium text-emerald-600 dark:text-emerald-400">No security findings were returned.</div>
                ) : (
                  [...safeArray(assessment.vulnerabilities), ...safeArray(assessment.findings)].map((item, index) => (
                    <div key={`${item.title}-${index}`} className="rounded-2xl border border-[var(--border)] bg-[var(--background)] p-5">
                      <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                        <h3 className="font-semibold">{item.title}</h3>
                        <span className="text-xs font-semibold uppercase tracking-[0.12em] text-[var(--accent)]">{item.severity}</span>
                      </div>
                      <p className="mt-3 text-sm leading-6 text-[var(--muted)]">{item.description}</p>
                      {"recommendation" in item && item.recommendation && <p className="mt-4 text-sm leading-6 text-[var(--text)]"><span className="font-semibold">Recommendation:</span> {item.recommendation}</p>}
                    </div>
                  ))
                )}
              </div>
