...
export async function POST(request: Request) {
  try {
    ...
    if (description.length < 10) {
      return NextResponse.json({ error: "Please provide more detail about the project." }, { status: 400 });
    }
    ...
    const adminEmailError = (
      await resend.emails.send({
        ...
        html: `
          <div style="font-family:Arial,sans-serif;max-width:700px;margin:auto;color:#111;line-height:1.7">
            ...
            <h2>Requirements</h2>
            <p>${escapeHtml(requirements || "Not provided").replace(/\n/g, "<br />")}</p>
          </div>
        `,
      })
    ).error;
    ...
    const clientEmailError = (
      await resend.emails.send({
        from: "OLYR Labs <hello@olyrlabs.com>",
        to: email,
        subject: "Your OLYR Labs Project Request Has Been Received",
        html: `
          <div style="font-family:Arial,sans-serif;max-width:700px;margin:auto;color:#111;line-height:1.7">
            <h1>Thank you for contacting OLYR Labs</h1>
            <p>Hello ${escapeHtml(name)},</p>
            <p>We have successfully received your project request. Our team will review your requirements and contact you regarding the next steps.</p>
            <hr />
            ...
            <p><strong>Timeline:</strong> ${escapeHtml(timeline || "Not provided")}</p>
            <hr />
            <p>We appreciate your interest in OLYR Labs. We will get back to you as soon as possible.</p>
            <p>Regards,<br />OLYR Labs</p>
          </div>
        `,
      })
    ).error;
