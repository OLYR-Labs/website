...
export async function POST(request: Request) {
  try {
    ...
    if (contentLength && Number.isFinite(Number(contentLength)) && Number(contentLength) > 16 * 1024) {
    ...
    try {
      ...
      const exposureAnalysis = await analyzeExposure(targetUrl, controller.signal).catch(() => ({
        findings: [],
        discoveredUrls: [],
        probedUrls: [],
        exposedUrls: [],
      ...
      return NextResponse.json({
        ...
        categories: {
          ...
          exposure: exposureAnalysis.findings.length ? 25 : 100,
        },
        ssl: sslAnalysis,
    ...
    const status = ["INVALID_TARGET", "INVALID_PROTOCOL", "CREDENTIALS_NOT_ALLOWED", "BLOCKED_TARGET", "PORT_NOT_ALLOWED"].includes(message)
      ? 400
      : message === "RESPONSE_TOO_LARGE" ? 413 : 502;
