...
function isBlockedHostname(hostname: string): boolean {
  ...
  return normalized === "localhost" || normalized.endsWith(".localhost") || normalized.endsWith(".local") || isPrivateIpv4(normalized) || isPrivateIpv6(normalized);
}
  ...
  try {
    ...
    try {
      ...
      const response = await fetch(target, {
        method: "GET",
        headers: { "User-Agent": "OLYR-SecureScan-Enterprise" },
        redirect: "error",
      ...
      const websiteScore = calculateSecurityScore([
        httpsScore,
      ...
      if (!sslAnalysis.valid) {
        findings.push({ title: "SSL Certificate", severity: "High", description: "Unable to verify SSL certificate." });
      }

      if (sslAnalysis.valid && sslAnalysis.daysRemaining < 30) {
        findings.push({ title: "SSL Certificate Expiration", severity: "Medium", description: `Certificate expires in ${sslAnalysis.daysRemaining} days.` });
      }

      if (!domainAnalysis.emailSecurity.SPF) {
        findings.push({ title: "Missing SPF Record", severity: "Medium", description: "Domain does not have SPF email protection configured." });
      }

      if (!domainAnalysis.emailSecurity.DMARC) {
        findings.push({ title: "Missing DMARC Protection", severity: "Medium", description: "Domain does not have DMARC email authentication configured." });
      }
      ...
      return NextResponse.json({
        ...
        overview: {
          ...
          grade: websiteScore >= 90 ? "Excellent" : websiteScore >= 75 ? "Good" : websiteScore >= 50 ? "Needs Improvement" : "Critical",
        },
        categories: {
          websiteSecurity: websiteScore,
    ...
    if (message !== "INVALID_TARGET" && message !== "BLOCKED_TARGET") {
